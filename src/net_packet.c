...
/* Minimum packet size to attempt compression.
   On routers with limited CPU, skipping tiny packets saves cycles
   with negligible bandwidth impact. Set to 0 to disable. */
#ifndef COMPRESS_MIN_SIZE
#define COMPRESS_MIN_SIZE 64
#endif
...
static length_t compress_packet(uint8_t *dest, const uint8_t *source, length_t len, int level) {
	...
	/* Tiny packets rarely benefit from compression; return 0 so caller
	   falls back to uncompressed send, saving CPU. */
	if(len < COMPRESS_MIN_SIZE) {
		return 0;
	}
